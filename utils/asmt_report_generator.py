...
from utils.globals.constants import bo_comparison_summary_dict, \
    gstr3b_analysis_dict, result_point_6, \
    gstr3b_merged_dict, result_point_14, result_point_15, result_point_16, result_point_17, \
    string_zero, string_yes
...
col_2_values = [
    ...
    "Difference in ITC availed in GSTR 3B vis a vis available in 2A/2B as per GSTR- 9 table 8D after countering "
    "effect of table 13",
    "Difference in GSTR-1 Outward Supply Vs Supply made in E-way bill",
    ...
    "Liability of Interest as per Rule 37",
    "Liability of Interest on reclaimed ITC",
...
async def asmt_10_report_generator(gstin, master_dict):
    print(f" === Started execution of method asmt_10_report_generator for: {gstin} ===")
    ...
    try:
        ...
        gstin_of_taxpayer = master_dict.get(gstr3b_analysis_dict, {}).get("gstin_of_taxpayer", "")
        ...
        doc.add_paragraph(
            '\nDear Taxpayer,\n\n'
            'During scrutiny of your returns furnished for the period FY 2021-22 under Section 61 of the CGST Act, '
            '2017, '
        ...
        # Populate column 3 based on master_dict values
